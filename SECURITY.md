...
2. **Safe defaults** — on first boot an API key is generated into the datastore; listens on loopback by default; **Safe Mode is on** (read-only) unless turned off in Settings (seeded off when `LISTARR_APPLY=1`).
